F.module("lib/MVP",['lib/A'],function(){
	// MVP构造函数
	var MVP =function(modeName,pst,data){
		// 实现通过MVP(模块名称、模块管理器、服务器端获取的数据)的方式添加模块
		// 在数据层中添加modeName渲染数据模块
		MVP.model.setData(modeName,data);
		// 在管理器层中添加modeName管理器模块
	...
	MVP.view = function(){
		...
		*/
		function getHTML(str,type){
			...
			// li.@mode @choose @last[data-mode=@mode]
			return str
