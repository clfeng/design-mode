		...
		*/
		,appendTo:function(parent){
			...
			if (doms.length) {
				// 遍历父元素
				for(var j =this.length-1;j >=0;j--){
					// 简化元素克隆(cloneNode)惭怍，只向第一个父元素中插入子元素
	...
	var Tween = {
		...
		*/
		loop:function(){
			...
			// 获取当前时间//+new Date()是new Date().getTime()的缩略写法
			var now = +new Date();
			//遍历运动成员
		...
		*/
		workFn:function(instance){
			// 获取当前成员在当前时刻下的运动进程
			instance.tween = this.easing[instance.type](instace.passed,intance.from,instance.to-instance.from,instance.duration);
			// 执行主函数
			this.exec(instance);
		},
		/***
